...
const registerValidator = async (req, res, next) => {
  ...
  try {
    ...
    if (username.toLowerCase().includes("kaushik")) {
      return res.status(400).json({
        error:
          "These Usernames (including 'kaushik') are reserved for Admin Only !",
      });
    }
...
const checkExecServiceAvailable = (req, res, next) => {
  if (
    process.env.NO_EXECUTION &&
    process.env.NO_EXECUTION.toLowerCase() === "true"
  ) {
    return res
      .status(503)
      .json({
